...
" \\___/|____/ \\___\\__,_|_| |_| |_|\n");
	...
	printf("OSCam is based on Streamboard mp-cardserver v0.9d - (w) 2004-2007 by dukat\n\n");
	...
	printf("\t-g <mode>  : Garbage collector *DEBUG MODE* (1=immediate free, 2=check for double frees).\n");
	printf("\t-w <secs>  : Wait up to <secs> seconds for the system time to be set correctly (default: 60).\n");
	if (config_WEBIF()) {
		printf("\t-r <level> : Set restart level\n");
		printf("\t         0 = Disabled\n");
		printf("\t         1 = Restart activate, webif can restart oscam (default)\n");
		printf("\t         2 = Like 1, but also restart on segmentation faults\n");
		printf("\t-u         : Enable UTF-8 usage in webif.\n");
	}
