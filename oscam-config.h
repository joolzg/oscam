...
/*
 * Functions that allow checking of configuration variables in
 * C code without using the preprocessor. The dead code elimination
 * of the compiler takes care of removing the code that depends
 * on disabled config but we can compile coverage even if some
 * config is disabled.
 */
